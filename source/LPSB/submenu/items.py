...
from scripts import j2, i, tracker, lpsql, memory, parser
            ...
            if exists(path_):
                ...
                if len(listdir_) > 0:
                    for k in listdir_:
                        ...
                        await callback.message.answer(f'<code>{parser.de_anchor(js["text"])} / {js["price"]}</code>')
                else:
        ...
        if len(parse) >= 2 and parse[-1].isnumeric():
            try:
                ...
                if p <= 0:
                    ...
                    parsed_name = parser.anchor('/'.join(parse[:-1]))
...
async def edit_item(message: Message, callback: CallbackQuery, state: FSMContext, n: int):
    try:
        ...
        if exists(path_) and len(listdir(path_)) > 0:
            ...
            new_string = f'<code>{parser.de_anchor(js["text"])} / {js["price"]}</code>'
