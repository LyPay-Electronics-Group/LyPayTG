...
from scripts import tracker, parser, messenger
...
async def r34(message: Message):
    ...
    await messenger.warn(text=txt.EXE.EVENTS.HIDDEN_STUFF_R34.format(id=message.from_user.id))
...
async def xilonen(message: Message):
    ...
    await messenger.warn(text=txt.EXE.EVENTS.HIDDEN_STUFF_XILONEN.format(id=message.from_user.id))
